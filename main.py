...
def clean_items(items):
    """Drop obviously malformed records before they are published."""
    cleaned = []
    seen = set()
    for c in items:
        start = (c.get("start_date") or "").strip()
        end = (c.get("end_date") or start).strip()
        title = (c.get("title") or "").strip()
        url = (c.get("url") or "").strip()
        if not title or not start or not url:
            continue
        # ISO dates sort lexically. Reject impossible ranges such as end before start.
        if end < start:
            continue
...
def upsert_collected(items):
    ...
    with db() as conn:
        for c in items:
            ...
            # Auto-publish everything unless you have explicitly rejected it in the admin page.
            status = "rejected" if old and old[0] == "rejected" else "verified"
            conn.execute("""
...
def refresh_courses(request: Request):
    ...
    return {"ok": True, "collected": len(clean_items(items)), "auto_published": True}
...
<body><h1>Course manager</h1><p>New courses are published automatically. Use Reject only when you want to remove a bad listing.</p><div class='bar'><a href='/admin?key={{KEY}}&status=pending'>Pending</a><a href='/admin?key={{KEY}}&status=verified'>Verified</a><a href='/admin?key={{KEY}}&status=rejected'>Rejected</a><form method='post' action='/api/refresh?key={{KEY}}'><button>Refresh sources</button></form></div>
